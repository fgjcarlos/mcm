...
type BootstrapAdminConfig struct {
	...
	// AutoGenerated is true when Load populated the credentials because
	// MCM_BOOTSTRAP_ADMIN_USERNAME / _PASSWORD were unset. Callers may use
	// this to log a one-shot warn line with the credentials.
	AutoGenerated bool `yaml:"-"`
}
...
func Default() Config {
	return Config{
		...
		Auth: AuthConfig{
			// JWTSecret and BootstrapAdmin are left empty so Load()'s
			// env-first path can auto-generate them on first boot per the
			// #228 contract. Callers that need a known-good config in
			// tests must set them explicitly (e.g. the helpers in
...
// It is a complete, valid sample (every required field populated) so an
// operator copying it as a starting point gets a config that passes
// Validate() and only needs edits. Because #228 makes the JWT secret and
// bootstrap admin auto-generated on boot, the example fills those in
// explicitly — leaving them empty in the sample would be confusing.
...
//     32-byte secret is generated. The secret is persisted to
//     <Database.Path's directory>/.bootstrap.json (mode 0600) so
//     subsequent restarts reuse it.
//   - If MCM_BOOTSTRAP_ADMIN_USERNAME / _PASSWORD were unset,
//     BootstrapAdmin.Username = "admin" and BootstrapAdmin.Password
//     is a random 24-char string. BootstrapAdmin.AutoGenerated is
//     set so the caller can log a one-shot warn line.
//
// Load then validates the result and returns it.
func Load(path string) (Config, error) {
	cfg := Default()

	yamlPath := path
	if yamlPath == "" {
		yamlPath = os.Getenv("MCM_CONFIG_FILE")
	}
	if yamlPath != "" {
	...
	if strings.TrimSpace(cfg.Auth.BootstrapAdmin.Username) == "" &&
		strings.TrimSpace(cfg.Auth.BootstrapAdmin.Password) == "" {
		pw, err := randomPassword(24)
		if err != nil {
			return Config{}, fmt.Errorf("generate bootstrap admin password: %w", err)
		}
		cfg.Auth.BootstrapAdmin.Username = "admin"
		cfg.Auth.BootstrapAdmin.Password = pw
		cfg.Auth.BootstrapAdmin.AutoGenerated = true
	}
...
// bootstrapState is the on-disk format of <database-dir>/.bootstrap.json.
// It carries values that must persist across restarts when no env var or
// YAML field is set (currently only the JWT secret).
type bootstrapState struct {
	JWTSecret string `json:"jwt_secret"`
}

// loadOrGenerateJWTSecret returns the persisted JWT secret if one is on
// disk; otherwise it generates a fresh 32-byte (64 hex-char) secret,
// writes it to <database-dir>/.bootstrap.json with mode 0600, and returns
// it. The caller logs a one-shot info line on generation.
//
// Persistence errors during write are non-fatal: the in-memory secret is
// still returned and the server can boot, but the next restart will
// generate a new secret and existing tokens will become invalid. The
// error is wrapped so the caller can decide.
func loadOrGenerateJWTSecret(databasePath string) (string, error) {
	statePath, err := bootstrapStatePath(databasePath)
	if err != nil {
		return "", err
	}
	...
	if data, err := os.ReadFile(statePath); err == nil {
		...
		if err := json.Unmarshal(data, &st); err == nil && st.JWTSecret != "" {
			return st.JWTSecret, nil
		}
	} else if !errors.Is(err, os.ErrNotExist) {
		// Surface unexpected read errors (permission denied, etc.) so the
		// operator notices — generating a new secret silently would
		// invalidate existing tokens without warning.
		return "", fmt.Errorf("read bootstrap state %q: %w", statePath, err)
	}
	...
	state := bootstrapState{JWTSecret: secret}
	...
	if err := os.WriteFile(statePath, data, 0o600); err != nil {
		return "", fmt.Errorf("write bootstrap state %q: %w", statePath, err)
	}
	return secret, nil
}

// bootstrapStatePath returns <dir>/.bootstrap.json where dir is the parent
// of the SQLite database file. The leading dot hides the file in normal
// directory listings.
func bootstrapStatePath(databasePath string) (string, error) {
	dir := filepath.Dir(databasePath)
	if dir == "" || dir == "." {
		return "", fmt.Errorf("database path %q has no parent directory for bootstrap state", databasePath)
	}
	return filepath.Join(dir, ".bootstrap.json"), nil
}

// randomHex returns n random bytes encoded as n*2 lowercase hex characters.
...
// randomPassword returns n random printable ASCII characters drawn from
// a 64-char unambiguous alphabet (lowercase, uppercase, digits, no
// look-alikes). The bootstrap admin password is shown in logs once on
// first boot, so we keep it copy-paste friendly.
